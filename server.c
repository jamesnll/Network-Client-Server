...
// Command Runner
static void split_input(const char *input, char **command, char **args, int *args_used);
...
int main(int argc, char *argv[])
{
    ...
    while(!exit_flag)
    {
        ...
        {
            free_memory(command, args, args_used);
            socket_close(client_sockfd);
            continue;
        }

        // Duplicate and redirect STDOUT
        stdout_copy = fcntl(STDOUT_FILENO, F_DUPFD_CLOEXEC, 0);
        redirect_stdout(client_sockfd);
        execute_process(full_path, args);

        // Redirect STDOUT back to original fd
 ...
 * Error cases:
 *
 * ip address still bound when program crashes
 * Client: macOS, Server: Fedora: connecting from macOS to fedora: connect: Invalid argument
 * Client: Fedora Server: macOS: accept failed: Invalid argument -> infinite loop
 *
 * Client: Fedora Server: Fedora accept: invalid argument
 *
 * error closing client_sockfd
 *
 *
 * Web Server: Chat Program
 *
 * Client makes a request
 * Server forks a child/ Makes a thread
 * Child does work
 * Sends work back to client
 *
 * Thread pools? I/O Multiplexing
 *
 *
 * Project
 *
...
/*
 * Testing across lab machines
 *
 * Client:      Server:     Works?
 * Linux        Linux       Yes
 * Linux        macOS       Yes
 * Linux        freeBSD     Yes
 * macOS        Linux       No -> connect: invalid argument
 * macOS        macOS       No -> connect: invalid argument
 * macOS        freeBSD     No -> connect: invalid argument
 * freeBSD      Linux       No -> connect: invalid argument
 * freeBSD      freeBSD     No -> connect: invalid argument
 * freeBSD      macOS       No -> connect: invalid argument
 */
