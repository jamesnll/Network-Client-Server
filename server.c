...
// Command Runner
static void split_input(const char *input, char **command, char **args, int *args_used, int stdout_copy);
...
int main(int argc, char *argv[])
{
    ...
    while(!exit_flag)
    {
        ...
        split_input(buffer, &command, args, &args_used, stdout_copy);

        if(find_binary_executable(command, full_path, stdout_copy) != 0)
        {
...
static void split_input(const char *input, char **command, char **args, int *args_used, int stdout_copy)
{
    ...
    if(input_copy == NULL)
    {
        fprintf(stdout, "Input copy memory allocation failed\n");
        reset_stdout(stdout_copy);
        exit(EXIT_FAILURE);
    }
    ...
    while(token != NULL)
    {
        ...
        if(args_count == 0)
        {
            *command = (char *)malloc(strlen(token) + 1);
            ...
            if(*command == NULL)
            {
                fprintf(stdout, "Command memory allocation failed\n");
                free(*command);
                reset_stdout(stdout_copy);
                exit(EXIT_FAILURE);
            }
        ...
        {
            fprintf(stdout, "Args index %d memory allocation failed\n", args_count);
            reset_stdout(stdout_copy);
            exit(EXIT_FAILURE);
        }

        strncpy(args[args_count], token, strlen(token));
        args[args_count][strlen(token)] = '\0';
